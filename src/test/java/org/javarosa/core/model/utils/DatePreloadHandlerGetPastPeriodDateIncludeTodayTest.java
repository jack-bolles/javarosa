...
public class DatePreloadHandlerGetPastPeriodDateIncludeTodayTest {
    ...
    public void includeTodayReturnsNextFurtherPeriodIfTrueAndBeginningIsFalseAndRefDateIsTheEndOfAPeriod() {
        ...
        assertEquals(dateFromLocalDate(LocalDate.of(2023, 6, 24)),
                week.pastPeriodFrom(endOfPeriod, "sun", false, !includeToday, -1));
        assertEquals(dateFromLocalDate(LocalDate.of(2023, 6, 17)),
                week.pastPeriodFrom(endOfPeriod, "sun", false, includeToday, -1));

        assertEquals(dateFromLocalDate(LocalDate.of(2023, 6, 3)),
                week.pastPeriodFrom(endOfPeriod, "sun", false, !includeToday, 0));
        assertEquals(dateFromLocalDate(LocalDate.of(2023, 6, 10)),
                week.pastPeriodFrom(endOfPeriod, "sun", false, includeToday, 0));

        assertEquals(dateFromLocalDate(LocalDate.of(2023, 5, 27)),
                week.pastPeriodFrom(endOfPeriod, "sun", false, !includeToday, 1));
        assertEquals(dateFromLocalDate(LocalDate.of(2023, 6, 3)),
                week.pastPeriodFrom(endOfPeriod, "sun", false, includeToday, 1));

        assertEquals(dateFromLocalDate(LocalDate.of(2023, 5, 20)),
